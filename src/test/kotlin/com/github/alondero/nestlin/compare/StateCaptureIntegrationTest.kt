...
class StateCaptureIntegrationTest {
    ...
    fun captureMesen2StateAtFrame60() {
        try {
            ...
            println("Mesen2 I/O access not enabled (${e.message}), skipping test")
    ...
    fun compareNestlinAndMesen2() {
        ...
        try {
            if (!Mesen2StateCapturer.isMesen2Available()) {
                println("Mesen2 not available, skipping comparison")
                return
            }
            ...
            println("Mesen2 I/O access not enabled (${e.message}), skipping comparison test")
            assumeTrue(e.message, false)
        }
    }
}
