...
- **Verification:**
  ...
  - `Mapper64KlaxRegressionTest` (Mesen2 state diff) — at the title-screen frames (60, 240) Klax's CHR and palette are **byte-identical to Mesen2** and the CPU PC matches within the capture-scanline offset. The attract sequence (Tengen logo, credits) renders correctly. Residual: small attract-loop timing drift by frame 600 (different attract screen than Mesen2) attributable to sub-scanline IRQ / the pre-NMI vs post-NMI capture offset — not the blank-screen bug, which is fixed.
