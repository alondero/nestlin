...
- **Enum access:** Use `Enum.entries` (Kotlin 1.9+) instead of `Enum.values()`. `values()` clones a fresh `Array<E>` on every call (defensive copy); `entries` returns the pre-allocated `EnumEntries<E> : List<E>` with zero allocation. For save-state deserialization, use the `DataInput.readEnum<T>()` helper in `SaveState.kt` — it bounds-checks the ordinal and raises `SaveState.IncompatibleSaveStateException` on a corrupted file (never `IndexOutOfBoundsException`). Sites that need to handle out-of-range ordinals themselves (e.g. Mapper34's variant-mismatch error needs the loaded name even on out-of-range input) read the ordinal manually and use `T.entries.getOrNull(ordinal)` directly. The rule is enforced by `KonsistArchitectureTest` (issue #314) — `./gradlew test` fails the build on any new `EnumClass.values()` call in production code.
- **Static analysis — Konsist (issue #314):** Architectural and Kotlin-idiom rules live in `src/test/kotlin/com/github/alondero/nestlin/testutil/KonsistArchitectureTest.kt`. Konsist was chosen over Detekt and ktlint because it runs as a JUnit test in the existing fast lane (`./gradlew test`) — no separate Gradle plugin to keep aligned with the project's pinned Kotlin 1.9.22 toolchain, and rules are Kotlin code next to existing tests, not YAML. Run a single Konsist test with `./gradlew test --tests "*KonsistArchitectureTest"`. Version is pinned to `0.13.0` (Oct 2023, Kotlin 1.9.x compiler); the 0.17.x line bumps its embedded compiler to 2.0.20, which conflicts with our 1.9.22 — see the comment in `build.gradle.kts`. **Adding a new rule:** copy an existing `@Test fun`, pick a scope (`Konsist.scopeFromProject()`, `scopeFromProduction()`, or `scopeFromTest()`), filter the declaration kind you care about, and `.assertTrue` / `.assertFalse`. Konsist 0.13.0 provides `KoFileDeclaration.text` for the rare synthesized-property rule (e.g. `Enum.values()`), `KoClassDeclaration.hasEnumModifier` / `resideInPackage(...)` for AST-only rules, and `KoImportProvider.imports` for cross-package checks. Keep rules on a single concept; if the signal is in the literal source bytes (e.g. `ByteArray(16)` header construction) keep the regex in the dedicated `*LintTest` file because the grandfathered-baseline machinery (`BASELINE_FAIL` etc.) is a separate concern.
